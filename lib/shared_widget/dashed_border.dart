...
class DashedBorder extends Border {
  ...
  void paint(
    ...
    if (borderRadius != null) {
      final Path path = Path()..addRRect(borderRadius.toRRect(rect));
  ...
  void _drawPath(Canvas canvas, Path source, Rect rect) {
    ...
    final space = spaceLength ?? ((spaceRatio ?? 1.0) * dashLength);
    //region calculate spacing and draw top border
    int partNumber = (topWidth) ~/ (dashLength + space);

    final topSpacing = (topWidth - (partNumber * dashLength)) / (partNumber);
    double distance = 0;
    ...
    final rightSpacing = (rightHeight - (partNumber * dashLength)) / (partNumber);
    distance = (topWidth);
    ...
    while (distance < (metric.length - bottomWidth - leftHeight)) {
      ...
      if (isOnlyCorner && distance < (metric.length - bottomWidth - leftHeight) - diff) {
    ...
    final bottomSpacing = (bottomWidth - (partNumber * dashLength)) / (partNumber);
    ...
    final leftSpacing = ((leftHeight) - (partNumber * dashLength)) / (partNumber);
  ...
  void _drawRadiusPath(
    ...
    //region calculate spacing and draw left border
    int partNumber = (leftHeight + bottomLeftCorner / 2 + topLeftCorner / 2) ~/ (dashLength + space);

    final leftSpacing =
        ((leftHeight + bottomLeftCorner / 2 + topLeftCorner / 2) - (partNumber * dashLength)) / (partNumber);
    ...
    //region calculate spacing and draw top border
    partNumber = (topWidth + topRightCorner / 2 + topLeftCorner / 2) ~/ (dashLength + space);

    final topSpacing = (topWidth + topRightCorner / 2 + topLeftCorner / 2 - (partNumber * dashLength)) / (partNumber);
    ...
    while (distance < (leftHeight + topWidth) + topLeftCorner + topRightCorner / 2) {
      ...
      if (isOnlyCorner && distance < (leftHeight + topWidth) + topLeftCorner + topRightCorner / 2 - diff) {
        distance = (leftHeight + topWidth) + topLeftCorner + topRightCorner / 2 - diff;
      } else {
        distance = target + topSpacing;
      }
    }
    ...
    //region calculate spacing and draw right border
    partNumber = (rightHeight + topRightCorner / 2 + bottomRightCorner / 2) ~/ (dashLength + space);
    final rightSpacing =
        (rightHeight + topRightCorner / 2 + bottomRightCorner / 2 - (partNumber * dashLength)) / (partNumber);
    ...
    while (distance < (metric.length - bottomWidth - bottomLeftCorner - (bottomRightCorner / 2))) {
      ...
      if (isOnlyCorner &&
          distance < (metric.length - bottomWidth - bottomLeftCorner - (bottomRightCorner / 2)) - diff) {
        distance = (metric.length - bottomWidth - bottomLeftCorner - (bottomRightCorner / 2)) - diff;
      } else {
        distance = target + rightSpacing;
      }
    ...
    //region calculate spacing and draw bottom border
    partNumber = (bottomWidth + bottomRightCorner / 2 + bottomLeftCorner / 2) ~/ (dashLength + space);
    final bottomSpacing =
        (bottomWidth + bottomRightCorner / 2 + bottomLeftCorner / 2 - (partNumber * dashLength)) / (partNumber);
    ...
    while (distance < metric.length - bottomLeftCorner / 2) {
      ...
      if (isOnlyCorner && distance < metric.length - bottomLeftCorner / 2 - diff) {
